					 ...
					 <input type="text" name="title" value="<?php echo ($_GET["title"]); ?>"  placeholder="标题">
						  ...
						  <select name="status"  >
									 ...
									 <select name="type"  >
										...
										<?php if(is_array($dreamType)): $i = 0; $__LIST__ = $dreamType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><option value="<?php echo ($d["id"]); ?>" ><?php echo ($d["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						...
						<table class="table table-bordered table-striped table-hover">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
									...
									<td> <a style="text-decoration:underline;" href="<?php echo U('Donor/item_detail',array('item_id'=>$m['id'],'type'=>2));?>"> <?php echo get_sum_money($m['id'],2);?>&nbsp;&nbsp;&nbsp;(<?php echo get_money_ratio($m['id'],2);?>%)</a></td>
									<td><?php echo get_dream_type($m['type']);?></td>
									<td>
										<?php if($m["return_content"] =='' && $m["return_money"] == ''): ?>无
											<?php else: ?>
												有回报<?php endif; ?>
									</td>
									<th>
										<?php if($m["is_top"] == 1): ?><span style="color: red">已置顶</span>
											<?php else: ?>
												未置顶<?php endif; ?>
										</th>
									...
									<td>
										<a class="btn btn-info" href="<?php echo U('Dream/edit',array('id'=>$m['id']));?>" role="button">修改</a>
...
function del(aid){
	...
	$.get("<?php echo U('Dream/del');?>", {id:aid},function(data){
		if(data.status == 1){

...
class WxPayDataBase
{
	...
	* @param string $value 
	**/
	public function SetSignTransfers()
	{
		$sign = $this->MakeSignTransfers();
		$this->values['sign'] = $sign;
		return $sign;
	}
	...
	**/
	...
	public function MakeSign()
	{
		...
		// var_dump($string);die;
		$string = md5($string);
		// 777842767707bbaa47aeb38102b3b66b
		//签名步骤四：所有字符转为大写
	...
	/**
	 * 生成签名MakeSignTransfers
	 * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
	 */
	public function MakeSignTransfers()
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
...
class WxPayResults extends WxPayDataBase
{
     ...
     * @throws WxPayException
     */
	public static function Init1($xml)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
...
{	
	...
	**/
	...
	/**
	* 判断商户系统内部的提现订单号,32个字符内、可包含字母, 其他说明见商户订单号是否存在
	* @return true 或 false
	**/

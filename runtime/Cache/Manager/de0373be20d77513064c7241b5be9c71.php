			...
			<div class="panel-body">
				<div class="row">
					...
					<div class="col-md-12 ">
					 ...
					 <input type="text" name="cu_id" value="<?php echo ($_GET["cu_id"]); ?>"  placeholder="ID">
					 <input type="text" name="username" value="<?php echo ($_GET["username"]); ?>"  placeholder="昵称">
					 <input type="text" name="phone" value="<?php echo ($_GET["phone"]); ?>"  placeholder="手机号">
					 <input type="text" name="auth_id" value="<?php echo ($_GET["auth_id"]); ?>"  placeholder="身份证号">
						...
						<table class="table table-bordered table-striped table-hover">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
									<td><?php echo ((isset($m["cu_id"]) && ($m["cu_id"] !== ""))?($m["cu_id"]):'未填'); ?></td>
									<td><?php echo ((isset($m["username"]) && ($m["username"] !== ""))?($m["username"]):'未填'); ?></td>
									<td><?php echo ($m["love_value"]); ?></td>
									<td><?php echo ((isset($m["phone"]) && ($m["phone"] !== ""))?($m["phone"]):'未填'); ?></td>
									<td><?php echo get_province($m['home_province']);?></td>
									<th><?php echo get_city($m['home_city']);?></th>
									<td>
										<?php if($m['is_auth'] == 1): ?><span style="color: green">【已认证】</span>
									...
									<td>
										<a class="btn btn-info" href="<?php echo U('User/edit',array('id'=>$m['id']));?>" role="button">修改</a>

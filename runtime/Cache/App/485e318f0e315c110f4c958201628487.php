...
<div class="detail">
    ...
    <style type="text/css"> .contt{display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 6; overflow: hidden; height: 2.73rem; padding-bottom: 0.2rem; } .detail .reward {padding: 0.2rem; border-bottom: 1px solid #ccc; } .detail .reward span {color: red; } .detail .reward p {margin-top: 0.1rem; } .detail .reward:last-child {border: none; } .detail .comment {width: 100%; display: flex; margin-top: 0.1rem; flex-direction: column; background: #eee; justify-content: flex-end; } .detail .comment p {padding: 0.2rem; } .detail .comment span {color: #2ab2f6; } body{position: relative; } .cover{position:absolute; width: 100%; height:100%; left: 0; top:0; background:rgba(100,100,100,.5); display: flex; align-items: center; justify-content: center; } .cover>div{width: 5rem; height: 4rem; padding: 0.4rem; background: #eee; border-radius: 0.1rem; display: flex; flex-direction: column; align-items: flex-end; } .cover h5{width: 100%; } .cover textarea{width: 100%; padding: 0.15rem; background: #fff; margin: 0.1rem 0; } .cover .num{color: #999; font-size: 0.2rem; } .cover .btnBox{width: 100%; display: flex; } .cover .btnBox span{flex-grow: 1; text-align: center; padding: 0.2rem; } </style>
    <main class="main">
        <ul class="unifo">
            <li>
                <p class="img1">
                    <img src="<?php echo get_user_img($itemInfo['user_id']);?>" alt="">
                </p>
               <?php echo ($itemInfo["title"]); ?>
            </li>
            <li>
                <i class="iconfont icon-shijian"></i>
                剩余<b><?php echo overplus_time($itemInfo['date']);?></b>天
            </li>
        </ul>
        <div class="top1">
                ...
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        <?php if(is_array($imgArray)): $i = 0; $__LIST__ = $imgArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($i % 2 );++$i;?><div class="swiper-slide img2">
                ...
                <div>
                    <p class="jin">
                        <span style="width: <?php echo get_money_ratio($itemInfo['id'],2);?>%"></span>
                        <i class="iconfont icon-woniu" style="left:<?php echo get_money_ratio($itemInfo['id'],2);?>%;"></i>
                    </p>
                    <p class="tex1">
                        <span>已筹款<?php echo get_sum_money($_GET['id'],2);?>元</span><span>需筹<?php echo ($itemInfo["target_money"]); ?>元</span>
                    </p>
                </div>
                <!--已经帮助的-->
                <div>
                    ...
                    <p class="nums"><span>已帮助<?php echo get_money_cont($_GET['id'],2);?>次</span></p>
                </div>
                <!--排行榜-->
                <div class="ranking">
                    <p class="left">
        ...
        <div class="bottom" style="margin-bottom:0.15rem;">
            <div class="top">
                <p>
                    <span>资金用途</span><span>Use of funds</span>
                </p>
            </div>
            <div class="reward"
                <p><?php echo ($itemInfo["money_purpose"]); ?></p>
            </div>
        </div>
        <div class="bottom" style="margin-bottom:0.15rem;">
            <div class="top">
                <p>
                    <span>回报内容</span><span>Reward content</span>
                </p>
            </div>
            <div class="reward">
                <p><span><?php echo ($itemInfo["return_money"]); ?></span>元</p>
                <p><?php echo ($itemInfo["return_content"]); ?></p>
            </div>
        </div>
        <div class="bottom">
            <div class="top">
               ...
               <?php if(is_array($donorList)): $i = 0; $__LIST__ = $donorList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><ul>
                    ...
                    <li>
                        ...
                        <p class="bob">
                            ...
                            <span><i class="iconfont icon-pinglun" attr='<?php echo ($v["id"]); ?>' ></i><?php echo get_comments_cnt($v['id'],2);?></span>
                        </p>
                        <div class="comment">
    ...
    <footer class="footer">
        <div class="foot">
             <a href="<?php echo U('index/report',array('id'=>$_GET['id'],'type'=>2));?>">
                    <span>
...
$('.icon-pinglun').click(function(){
    ...
    $('.write').click(function(){
     ...
     $.ajax({
                ...
                data:{'donor_id':id,'type':2,'content':text,'item_id':<?php echo ($_GET["id"]); ?>},
    ...
    $('.icon-guanzhu').click(function(){
        ...
        history.go(0) 
         $.ajax({
                ...
                data:{'item_id':<?php echo ($_GET["id"]); ?>,'flag':'add','type':2},
                success:function(msg){
                    console.log(msg)
                    if(msg == -1){
                        alert('请先进行捐赠才可以评论！')
    ...
    $('.icon-guanzhu1').click(function(){
        ...
        $.ajax({
                type:'POST',
                url :"<?php echo U('index/follow');?>",
                data:{'item_id':<?php echo ($_GET["id"]); ?>,'flag':'delete','type':2},
                success:function(msg){
                    console.log(msg)
                if(msg != 0){

						...
						<table class="table table-bordered table-striped table-hover">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php echo ((isset($m["item_count"]) && ($m["item_count"] !== ""))?($m["item_count"]):'未填'); ?></td>
									<td><?php echo ($m["single_price"]); ?></td>
									<td><?php echo ($m['item_count']*$m['single_price']); ?></td>
									<td><?php echo get_province($m['province']);?> - <?php echo get_city($m['city']);?></td>
...
function del(aid){
	...
	$.get("<?php echo U('Announce/del');?>", {id:aid},function(data){
		if(data.status == 1){

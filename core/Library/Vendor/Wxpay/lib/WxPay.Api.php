...
//		 var_dump($inputObj);die;
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		//检测必填参数
		//
		 $inputObj->SetOp_user_id(WxPayConfig::MCHID);//操作者，商户号
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
	...
	/**
	 * 
	 * 微信企业提现到个人零钱，WxPayRefund中out_trade_no、transaction_id至少填一个且
	 * out_refund_no、total_fee、refund_fee、op_user_id为必填参数
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayRefund $inputObj
	 * @param int $timeOut
	...
	public static function transfers($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsPartner_trade_noSet()) {
			throw new WxPayException("提现零钱申请接口中，缺少必填参数提现订单号partner_trade_no！");
		}else if(!$inputObj->IsCheck_nameSet()){
			throw new WxPayException("提现零钱申请接口中，缺少必填参数check_name！");
		}else if(!$inputObj->IsAmountSet()){
			throw new WxPayException("提现零钱申请接口中，缺少必填参数amount！");
		}else if(!$inputObj->IsDescSet()){
			throw new WxPayException("提现零钱申请接口中，缺少必填参数desc！");
		}else if(!$inputObj->IsSpbill_create_ipSet()){
			throw new WxPayException("提现零钱申请接口中，缺少必填参数spbill_create_ip！");
		}
		...
		// dump($xml);die;
		$response = self::postXmlCurl($xml, $url, true, $timeOut);
		$result = WxPayResults::Init1($response);
		// dump($result);die;
		// array(7) {
		//   ["return_code"] => string(7) "SUCCESS"
		//   ["return_msg"] => string(12) "支付失败"
		//   ["mch_appid"] => string(18) "wx34eca8d0d3447ce5"
		//   ["mchid"] => string(10) "1492860912"
		//   ["result_code"] => string(4) "FAIL"
		//   ["err_code"] => string(9) "NOTENOUGH"
		//   ["err_code_des"] => string(12) "余额不足"
		// }
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
		return $result;
	}
	...
	public static function sendRedPack($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsMch_billnoSet()) {
			throw new WxPayException("现金红包申请接口中，缺少必填参数提现订单号mch_billno！");
		}else if(!$inputObj->IsSend_nameSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数send_name！");
		}else if(!$inputObj->IsRe_openidSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数re_openid！");
		}else if(!$inputObj->IsTotal_amountSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数total_amount！");
		}else if(!$inputObj->IsTotal_numSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数total_num！");
		}else if(!$inputObj->IsWishingSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数wishing！");
		}else if(!$inputObj->IsClient_ipSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数client_ip！");
		}else if(!$inputObj->IsAct_nameSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数act_name！");
		}else if(!$inputObj->IsRemarkSet()){
			throw new WxPayException("现金红包申请接口中，缺少必填参数remark！");
		}
		$inputObj->SetWxappid(WxPayConfig::APPID);//红包公众账号ID
		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		$inputObj->SetSign();//签名
		// dump($inputObj);die;
		...
		// array(10) {
		//   ["return_code"] => string(7) "SUCCESS"
		//   ["return_msg"] => string(57) "帐号余额不足，请到商户平台充值后再重试"
		//   ["result_code"] => string(4) "FAIL"
		//   ["err_code"] => string(9) "NOTENOUGH"
		//   ["err_code_des"] => string(57) "帐号余额不足，请到商户平台充值后再重试"
		//   ["mch_billno"] => string(23) "17122600720171228105911"
		//   ["mch_id"] => string(10) "1492860912"
		//   ["wxappid"] => string(18) "wx34eca8d0d3447ce5"
		//   ["re_openid"] => string(28) "oqscZ1WDSm5ZC4g068BDTbAFm4ns"

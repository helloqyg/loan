			...
			<div class="panel-body">
				<div class="row">
					...
					<div class="col-md-12 ">
					 ...
					 <input type="text" name="ins_name" value="<?php echo ($_GET["ins_name"]); ?>"  placeholder="机构名称">
						...
						<table class="table table-bordered table-striped table-hover">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
									<td><?php echo ($m["ins_name"]); ?></td>
									<td><?php echo ((isset($m["title"]) && ($m["title"] !== ""))?($m["title"]):'未填'); ?></td>
									<td><?php echo ($m["target_money"]); ?></td>
									<td> <a style="text-decoration:underline;" href="<?php echo U('Donor/item_detail',array('item_id'=>$m['id'],'type'=>3));?>"> <?php echo get_sum_money($m['id'],3);?>&nbsp;&nbsp;&nbsp;(<?php echo get_money_ratio($m['id'],3);?>%)</a></td>
									<td>
									...
									<td>
										<?php if($m["status"] == 3): ?>筹款中
											...
											<?php elseif($m["status"] == 2): ?>隐藏
											<?php else: ?> 审核中<?php endif; ?>
									</td>
									<td>
										<a class="btn btn-info" href="<?php echo U('Relaxed/add',array('id'=>$m['id']));?>" role="button">修改</a>
...
<script type="text/javascript">
	...
	$('.aaa').click(function(){
		window.location.href="<?php echo U('Relaxed/add');?>"
	})
...
function del(aid){
	...
	$.get("<?php echo U('Relaxed/del');?>", {id:aid},function(data){
		if(data.status == 1){
